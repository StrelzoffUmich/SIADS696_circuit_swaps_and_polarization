...
def label_one_batched(
    ...
    for entry in mirror_circuits:
        ...
        if mir is None:
            failed.append(dict(rec_base, seed=seed, status="route_fail",
                               err=extra or "route_fail"))
        else:
    ...
    for (Nact, _), group in sim_groups.items():
        ...
        ideal_peak = float(
            np.abs(
                Statevector(
                    group[0][1].remove_final_measurements(inplace=False)
                ).data
            ).max() ** 2
        )
        ...
        # seed_simulator per circuit restores the original's deterministic shot
        # sampling: given identical (circuit, transpiler_seed, noise_model), two
        # runs produce bit-for-bit identical count dicts.
        job = nsim.run(batch_circuits, shots=shots, seed_simulator=seed_list)
...
def _worker(args_tuple):
    ...
    """
    (
        file_path, algo, device_name, shots, seeds, opt,
        n_lo, n_hi,    # kept for filtering consistency but circuit is already filtered
    ) = args_tuple
    ...
    try:
        ...
        return [(dict(file=Path(file_path).name, algo=algo, device=device_name,
                      status="error", err=type(e).__name__), Path(file_path).name)]
    ...
    return [(dict(rec, file=fname, algo=algo, device=device_name), fname)
            for rec in recs]
...
def main():
    ...
    fieldnames = [
        "file", "algo", "n_qubits", "device", "seed",
        "status", "polarization", "polarization_raw",
        "p_success", "n_active", "routed_2q", "mirror_depth",
        "ideal_peak", "err",
    ]
    ...
    for algo, n, f in files:
        ...
        pending_seeds = [
            s for s in seeds
            if (fname, device_name, str(s)) not in done
        ]
        if not pending_seeds:
            continue
        work_items.append((
            str(f), algo, device_name,
            args.shots, pending_seeds, args.opt,
            args.n_lo, args.n_hi,
        ))
        ...
        with ProcessPoolExecutor(max_workers=n_workers) as pool:
            ...
            for fut in as_completed(futures):
                try:
                    ...
                    algo = item[1]
                    w.writerow(dict(
                        file=Path(item[0]).name, algo=algo,
                        device=device_name, status="error", err=type(e).__name__,
                    ))
                    fh.flush()
                    done_n += 1
                else:
