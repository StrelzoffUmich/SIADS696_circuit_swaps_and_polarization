...
The training process creates augmented versions of each graph with improved edges to help the model learn
...
def qasm_to_graph(qasm_str, gate_vocab):
    ...
    Converts a QASM circuit into a PyTorch Geometric graph representation.
        ...
        - Edge features encode special gate types (e.g., CX, measure, barrier)
            ...
            - these specifically are key QASM operations (CX, measure, barrier, ect)
    ...
    # Map nodes to indices (IMPORTANT: avoids O(n^2) ops.index calls)
    node_to_idx = {node: i for i, node in enumerate(ops)}

    num_gates = len(ops)

    #######################
    # Node features now using structural and topological
    # goal to give a larger, more rich neighborhood for each node in graph
    #######################
    x = []

    for i, node in enumerate(ops):
        gate_name = node.op.name
        gate_id = gate_vocab.get(gate_name, 0)

        num_qubits = len(node.qargs)

        # structural position in circuit
        relative_pos = i / max(num_gates, 1)

        # DAG structural context - https://quantum.cloud.ibm.com/docs/en/api/qiskit/qiskit.dagcircuit.DAGCircuit
        # backward look through the DAG
        pred_count = sum(1 for p in dag.predecessors(node) if p in node_to_idx)
        # forward look through the DAG
        succ_count = sum(1 for s in dag.successors(node) if s in node_to_idx)

        # one-hot for each gate type
        gate_onehot = np.zeros(len(gate_vocab), dtype=np.float32)
        gate_onehot[gate_id] = 1.0

        features = np.concatenate(
            [
                gate_onehot,  # gate one-hot identity
                [
                    num_qubits,  # important for 1 v 2 qubits
                    relative_pos,  # position in circuit
                    pred_count,  # dependency richness of prediction
                    succ_count,  # fan-out
                ],
            ]
        )
    ...
    #######################
    # Edge features, this time aware of dependencies around them
    # this is based on my knowledge of how quantium circuits work, but I could honestly be misunderstanding some things
    #######################
    ...
    for node in ops:
        ...
        for pred in dag.predecessors(node):
            ...
            src = node_to_idx[pred]

            # structural dependency strength based on how much they share in common
            # like atoms in a molecule having multiple bonds... if I'm understand correctly
            shared_qubits = len(set(pred.qargs) & set(node.qargs))

            # temporal distance in circuit execution order (bigger distance, weaker effcet)
            temporal_dist = dst - src

            edge_index.append([src, dst])

            edge_attr.append(
                [
                    shared_qubits,  # interaction with or entanglement strength proxy between nodes
                    temporal_dist,  # how far apart operations are in time
                ]
            )

    #######################
    # handel empty cases / graphs
    #######################
    ...
    #######################
    # build the pytorch gemotric data object
    #######################
    data = Data(x=x, edge_index=edge_index, edge_attr=edge_attr)

    #######################
    # add graph level metadata that includes global circuit attributes
    # to be used downstream possibly for clustering
    #######################
    data.depth = torch.tensor([dag.depth()], dtype=torch.float)
    data.num_qubits = torch.tensor([qc.num_qubits], dtype=torch.float)
    data.num_gates = torch.tensor([num_gates], dtype=torch.float)
    data.cx_count = torch.tensor(
        [sum(n.op.name == "cx" for n in ops)], dtype=torch.float
    )
...
def run_graphcl_pipeline(
    ...
    for model_name, encoder_cls in ENCODERS.items():
        ...
        # adding +4 for the 4 circuit wide features we added in qasm_to_graph()
        in_channels = len(gate_vocab) + 4
        model = GraphCL(encoder_cls(in_channels=in_channels), dropout=DROPOUT).to(
            device
        )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 4:
        print(
            "Usage: python embeddings_with_aug_dropout_and_norm_with_improved_edges.py <csv_path> <qasm_dir> <output_dir>"
        )
        sys.exit(1)
